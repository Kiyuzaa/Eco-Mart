...
<head>
  ...
  {{-- Custom utilities: gunakan Play CDN mode --}}
...
<div class="min-h-screen flex">
  ...
  >
    ...
    <div class="h-16 flex items-center justify-center border-b border-slate-700">
      <span class="text-lg font-extrabold tracking-wide text-emerald-400">Admin Panel</span>
    </div>
    ...
    <header class="sticky top-0 z-20 h-16 bg-white/90 backdrop-blur border-b border-slate-200">
      <div class="h-full px-4 sm:px-6 flex items-center justify-between">
        <div class="flex items-center gap-3">
          ...
          <button id="openSidebar" class="lg:hidden inline-flex items-center justify-center rounded-md p-2 text-slate-600 hover:bg-slate-100 focus:outline-none focus:ring-2 focus:ring-emerald-500" aria-label="Open sidebar">
            <svg xmlns="http://www.w3.org/2000/svg" class="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.6" d="M4 6h16M4 12h16M4 18h16"/>
            </svg>
          </button>
          <div>
            <div class="text-base font-semibold text-slate-800">@yield('header-title', 'Dashboard')</div>
            <div class="text-xs text-slate-500">@yield('header-subtitle', 'Welcome to your dashboard')</div>
          </div>
        </div>

        <div class="flex items-center gap-4">
          @yield('header-button')
          <button class="relative p-2 rounded-full hover:bg-slate-100 focus:outline-none focus:ring-2 focus:ring-emerald-500" aria-label="Notifications">
          ...
          <img class="w-9 h-9 rounded-full border border-slate-200" src="https://ui-avatars.com/api/?name=Admin&background=10b981&color=fff" alt="Admin avatar">
        </div>
      </div>
    </header>
...
<script>
  ...
  function openSidebar() {
    ...
    // pindah sidebar jadi offcanvas di mobile
